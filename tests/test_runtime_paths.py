...
from ssh_manager.config.loader import resolve_config_path
from ssh_manager.config.models import ManagerConfig
from ssh_manager.config.resolver import resolve_manager_config
from ssh_manager.runtime.paths import DATA_ROOT_ENV, DATA_ROOT_MARKER, resolve_data_root
...
def test_load_resolved_manager_config_uses_absolute_config_parent_as_data_root(tmp_path: Path) -> None:
    ...
    assert loaded.managed_config_path == (data_root / "ssh-home" / "managed" / "ssh-manager.conf").resolve()
...
def test_resolve_data_root_falls_back_to_marker(tmp_path: Path) -> None:
    home = tmp_path / "home"
    home.mkdir()
    ...
    resolved = resolve_data_root(cwd=marker_root, home=home, env={})

    assert resolved == marker_root.resolve()
