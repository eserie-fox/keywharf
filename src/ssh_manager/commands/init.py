...
from ssh_manager.commands.context import get_cli_state
from ssh_manager.services.init import analyze_init_root_requirements, initialize_workspace
...
def register(app: typer.Typer) -> None:
    ...
    def init_command(
        ...
        data_root: Path | None = typer.Option(
            None,
            "--data-root",
            help="Target data root to initialize.",
            file_okay=False,
            dir_okay=True,
        ),
        ...
        """Create a minimal ssh-manager workspace."""

        cli_state = get_cli_state(ctx)
        resolved_data_root = data_root.expanduser().resolve() if data_root is not None else None
        invocation = build_command_invocation(ctx, overrides={"data_root": resolved_data_root})
