...
def build_host_config(choice: SSHHostConfigChoice) -> SSHHostConfig:
    ...
    if not host.server_name:
        raise SSHManagerError("Remote host definition is missing ServerName")
    if not host.endpoints:
        raise SSHManagerError(f"Config '{host.server_name}' has no endpoint options.")
    if not host.authentication:
        raise SSHManagerError(f"Config '{host.server_name}' has no authentication options.")
